...
class GUI:
    ...
    def __init__(self, 
        ...
        self.timer = Timer()
        self.scene = Scene(dataset, self.gaussians, load_iteration=ckpt_it[0])
        self.timer.start()
    ...
    def register_dpg(self):
        ...
        with dpg.window(
            ...
            with dpg.collapsing_header(label="Rendering", default_open=True):
                ...
                def generate_image_pair(sender):
                    self.show_train_cam = not self.show_train_cam
                    
                with dpg.group(horizontal=True):
                    dpg.add_button(label="Depth", callback=callback_toggle_show_depth)
                    dpg.add_button(label="Generate Pair", callback=generate_image_pair)
    ...
    def save_view(self, buffer_image, name):
        from PIL import Image
        left_img = (buffer_image.permute(1, 2, 0)
                    .contiguous()
                    .clamp(0, 1)
                    .contiguous()
                    .detach()
                    .cpu()
                    .numpy())
        pil_image = Image.fromarray((left_img * 255).astype(np.uint8))
        pil_image.save(f'image_pairs/{name}.png')

    @torch.no_grad()
    def viewer_step(self):


        if self.show_train_cam:
            # Distance difference between left and center and right and center images
            baseline = 0.5
            ...
            self.save_view(img, 'center')

            # left
            camera.T[0] = camera.T[0] + baseline
            ...
            disp_left = (-baseline * fx) / img['depth']

            recon_img = warp_left_to_right(img['render'], disp_left)
            self.save_view(recon_img, 'left_recon')

            self.save_view(img['render'], 'left')

            # right
            camera.T[0] = camera.T[0] - (2.*baseline)
            camera.update_projections()
...
def warp_left_to_right(left_image, disparity_map):
    """
    Warp the left image to reconstruct the right image using the disparity map.

    Args:
        left_image (torch.Tensor): Left image of shape (C, H, W).
        disparity_map (torch.Tensor): Disparity map of shape (H, W).

    Returns:
        torch.Tensor: Approximated right image of shape (C, H, W).
    """
    C, H, W = left_image.shape  # Get image dimensions

    disparity_map = disparity_map.squeeze(0)


    # Create normalized mesh grid (-1 to 1) for the left image
    y_coords, x_coords = torch.meshgrid(
        torch.linspace(-1, 1, H, device=left_image.device),
        torch.linspace(-1, 1, W, device=left_image.device),
        indexing='ij'
    )

    # Convert disparity map to pixel displacement in normalized coordinates
    x_disparity = (disparity_map / W) * 2  # Normalize to [-1, 1] range

    # Compute right image coordinates by shifting left coordinates
    x_right = x_coords - x_disparity  # Shift pixels to the right
    y_right = y_coords  # Y-coordinates remain the same

    # Stack into a 2D flow grid (H, W, 2)
    grid = torch.stack((x_right, y_right), dim=-1)

    # Use grid_sample to warp the left image
    right_image = F.grid_sample(left_image.unsqueeze(0), grid.unsqueeze(0), mode='bilinear', padding_mode='border',
                                align_corners=True)

    return right_image.squeeze(0)  # Remove batch dimension
...
if __name__ == "__main__":
    ...
    assert args.configs, 'No config provided'
